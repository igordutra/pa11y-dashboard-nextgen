...
async function main() {
    console.log('\n--- Pa11y Dashboard NextGen: Setup Wizard ---');
    console.log('This script will help you configure your environment and set up an Admin account.\n');

    const config = await prompts([
        {
            type: 'confirm',
            name: 'authEnabled',
            message: 'Enable Authentication? (Highly recommended for production)',
            initial: true
        },
        {
            type: 'confirm',
            name: 'demoMode',
            message: 'Enable Demo Mode? (Disables background scans and restricts modifications)',
            initial: false
        }
    ]);
    ...
    try {
        ...
        console.log('\n' + '='.repeat(60));
        console.log('IMPORTANT: Add/Update the following in your .env file:');
